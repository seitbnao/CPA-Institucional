function Resultados(prova)
{
 	        $.ajax({
                method: 'GET',
                url: './get-resultado.php',
                dataType: 'json',
            data: {
                id_prova: prova
            },
...
function NovoAluno() {
        ...
        var elemento = document.getElementById('input_aluno');
        $.ajax({
            method: 'POST',
            url: './post-novoaluno.php',
            ...
            success: function (r) {
				...
				if(r.status == 'ok')
				{
					 window.location.href = '?page=avaliacao';
					 return;
				}
                 alert(r.message);
            }
...
$(function () {
    ...
    function submeteProva() {
        ...
        clearInterval(timer);
		 window.location.href = '?page=novaprova';
    }
    ...
    $('#comecar').on('click', function (e) {
	    ...
	    /**
     * Procedimento para iniciar o relatorio
     */
    $('#mostrarrelatorio').on('click', function (e) {
        ...
        $('.begin').fadeOut();
            ...
            success: function (r) {
				if(r.status = 'ok')
				{
					if(r.empty == true)
					{
						r.conteudo = "Não há relatório desta prova";
					}
					document.getElementById("resultados_fnais").innerHTML = r.conteudo ;
				}
			}
        });
